...
Kernel::Initialize()
{
    ...
    counter = 0; // 12/15 added for Aging.

    frameTable[128] = {0};      //IDKQQ Dont giveup!
...
int Kernel::Exec(char* name, int Initial_Priority)
{
    ...
    // orig_thread->setStatus(BLOCKED);
    // kernel->currentThread = t[threadNum];
    // currentThread->setStatus(RUNNING);
	t[threadNum]->space = new AddrSpace();
    // currentThread->setStatus(BLOCKED);
    // kernel->currentThread = orig_thread;
    // currentThread->setStatus(RUNNING);
    DEBUG(dbgAddr, "Create thread"<<name);
