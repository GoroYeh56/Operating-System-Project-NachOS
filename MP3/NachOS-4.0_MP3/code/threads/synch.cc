...
Semaphore::P()
{
    ...
    ///////// TODO : How to deal with theads if ALL threads are in BLOCKED.
    // How should they be added to Ready-queue after V()?
    // cout<<"\nThread "<<kernel->currentThread->getName()<<" want resource\n";
    #ifdef DEBUG_WAIT_QUEUE
    cout<<"\nThread "<<kernel->currentThread->getName()<<" want resource "<<this->getName() <<" , value: "<<value<<"\n";
    cout<<"WAIT queue ["<<this->getName()<<"] has "<<queue->NumInList()<<" threads\n";
    #endif
    
    while (value == 0) { 		// semaphore not available
	// cout<<"Append "<<currentThread->getName()<<" to semaphore waiting queue\n";
    // if(!queue->IsInList(currentThread)) queue->Append(currentThread);	// so go to sleep
	

        #ifdef DEBUG_WAIT_QUEUE
        cout<<"RUN: t"<<kernel->currentThread->getID()<<", waitFor->P() : Block in while(value==0)\n";
        cout<<"Num of t in WAIT queue ["<<this->getName()<<"] :"<<queue->NumInList();
        if(!queue->IsEmpty()) cout<<" Front: "<<queue->Front()->getName();
        else cout<<" empty\n";
        #endif
        ...
        #ifdef DEBUG_WAIT_QUEUE
        // List queue's content
        cout<<"\nAfter Append: ";
        cout<<"WAIT queue ["<<this->getName()<<"] : ";
        ListIterator<Thread*> *iter;
        iter = new ListIterator<Thread*>(queue);
    ...
    // cout<<"Sleep(FALSE) done, shoule not reach?\n";
    // error here!
    } 
...
Semaphore::V()
{
	...
	//  Original : kernel->scheduler->ReadyToRun(queue->RemoveFront());
        #ifdef DEBUG_WAIT_QUEUE
        // List queue's content
        cout<<"RUNNING : "<<kernel->currentThread->getName()<<", ";
        cout<<"Should Signal: ";
        cout<<"WAIT queue ["<<this->getName()<<"] :";
        ListIterator<Thread*> *iter;
        iter = new ListIterator<Thread*>(queue);
        ...
        #ifdef DEBUG_WAIT_QUEUE
        cout<<"Resources come, thread "<<thread->getName()<<" wake up and be added to ready-queue\n";
        cout<<"Cur thread priority: "<<kernel->currentThread->priority<<", new thread added priority: "<<thread->priority<<"\n";
        #endif
        kernel->scheduler->AddToQueue(thread, thread->priority);

        // if(  (kernel->currentThread->InWhichQueue()==2 && thread->InWhichQueue()==1) ||  ( kernel->currentThread->InWhichQueue()==3 && (thread->InWhichQueue()==2||thread->InWhichQueue()==1)) ){
        //     cout<<"\nShould Preempt!\n";
        ...
        //     cout<<"Add  priority: "<< thread->priority<<" belongs to queue "  <<thread->InWhichQueue()<<"\n"; 
            // kernel->scheduler->AddToQueue(kernel->currentThread, kernel->currentThread->priority); ///// KEY !!!!
        //     kernel->scheduler->Scheduling(FALSE);
        // }
