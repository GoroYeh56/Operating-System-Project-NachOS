...
SynchConsoleOutput::PutChar(char ch)
{
    ...
    waitFor->P();
    // cout<<"Release lock by thread: "<<kernel->currentThread->getID()<<"\n";
    lock->Release();
}
...
SynchConsoleOutput::CallBack()
{
    ...
    DEBUG(dbgTraCode, "SynchConsoleOutpu CallBack! call semaphore->V() ");
    DEBUG(dbgTraCode, "Cur WAIT queue: "<<waitFor->queue->Front()->getName());
    DEBUG(dbgTraCode, "RUNNING thread: "<<kernel->currentThread->getName());
    // cout<<"SynchConsoleOutput:: Callback()\n";
    // kernel->scheduler->List_All_thread();
    waitFor->V();
}
