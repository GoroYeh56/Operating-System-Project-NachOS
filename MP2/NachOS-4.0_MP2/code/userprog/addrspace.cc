...
AddrSpace::~AddrSpace()
{
    ...
    DEBUG(dbgAddr, "Free frame used by thread: "<<kernel->currentThread->getName());
    delete pageTable;
}
...
// how big is address space?
    ...
    ///////////////// Added at 11/08 23:59 by Goro Yeh ////////////
    pageTable = new TranslationEntry[numPages]; // for this user program.
    // pageTable = new TranslationEntry[NumPhysPages];
    for (int i = 0; i < numPages; i++)
    {
        // DEBUG(dbgAddr, "Thread "<<kernel->currentThread->getName()<< " finding  virtualPage" << i);
        for (int j = 0; j < NumPhysPages; j++)          //IDKQQ
        {
            if (kernel->frameTable[j] == 1){            //fame is used
                continue;
            }
            else{
                                                       //frame can be used
                ...
                kernel->NumFreeFrame--;
                DEBUG(dbgAddr, "Thread "<<kernel->currentThread->getName()<< " virtualPage" << i <<" uses frame "<<j);                
                break;
            }
    ...
    // int init_pageNumber =  noffH.initData.virtualAddr/PageSize;
    // int init_offset = noffH.initData.virtualAddr%PageSize;
    // int init_physical_address = pageTable[init_pageNumber].physicalPage*PageSize + init_offset;
    ...
    DEBUG(dbgAddr, "Thread "<<kernel->currentThread->getName()<<" done allocating page table.");
