...
static void Copy(char *from, char *to)
{
    ...
    DEBUG(dbgFile, "main: Open: "<<to_copied);
    openFile = kernel->fileSystem->Open(to_copied); // orig: to
...
void Print(char *name)
{
    ...
    int i, amountRead;
    char *buffer;
    DEBUG(dbgFile,"Open file: "<<name<<" and print out its content." );
    // std::cout<<"Open file: "<<name<<" and print out its content.\n";
    ...
    DEBUG(dbgFile,"I/O transfer from disk to CPU buffer.");
    DEBUG(dbgFile,"Contents in file:");
    buffer = new char[TransferSize];
...
//----------------------------------------------------------------------
static void CreateDirectory(char *name) //name: 
{
    DEBUG(dbgFile, "Create directory: "<<name<<" with size: "<<NumDirect * sizeof(DirectoryEntry));
...
int main(int argc, char **argv)
{
    ...
    if (threadTestFlag)
    {
        kernel->ThreadSelfTest(); // test threads and chronization
    }
    if (consoleTestFlag)
    {
    ...
    {   /* ------- MP4 ---------- */
        if(recursiveRemoveFlag) kernel->fileSystem->Remove(removeFileName, TRUE);
        else kernel->fileSystem->Remove(removeFileName, FALSE);
    }
    ...
    if (dirListFlag)
    {
        ...
        if (recursiveListFlag){
            ...
            // kernel->fileSystem->List(listDirectoryName , TRUE);  //TRUE means recursive!
        }
        else kernel->fileSystem->List(listDirectoryName , FALSE);
        // kernel->fileSystem->List();
    }
    if (mkdirFlag)
    {
        // kernel->fileSystem->CreateDirectory(createDirectoryName);
