...
Bitmap::Bitmap(int numItems)
{
    ...
    DEBUG(dbgFile, "New bitmap has "<<numBits<<" bits. (# free sectors)");

    numWords = divRoundUp(numBits, BitsInWord);
    map = new unsigned int[numWords]; // Set 512000 buffer here!
...
{
    // 不得超出 bitmap 擁有的 entry index 範圍！
    ASSERT(which >= 0 && which < numBits);

    map[which / BitsInWord] |= 1 << (which % BitsInWord);
    // since map is an integer(4 bytes == a Word) array,
    // divided by 32 bits to find the entry, and
    // mod 32 to find which specific 'bit' should be set to one!(use OR |=) 
    ASSERT(Test(which));
}
...
int Bitmap::FindAndSet()
{
    for (int i = 0; i < numBits; i++)
    {
        if (!Test(i))
        {
            Mark(i);  // Note,  內部會再 convert成 bitmap 的int map[]的 第幾個entry，的第幾個bit！！！ 
            return i; // 這邊回傳第幾個bit被set，表示拿到第幾個 sector (sector number)
                      // 類似 virtual memory 在找free frame的概念！
        }
    }
    return -1;
}
...
{
    cout << "Bitmap set:(has been allocate)\n";
    for (int i = 0; i < numBits; i++)
    {
        if (Test(i))
        {
