...
public:
    ...
    /* MP4 */
    bool isDir; // TRUE: is directory , FALSE: is a file.
...
/*---------- MP4---------------*/
    ...
    // New a entry for a 'file' or a 'sub-directory'
    ...
    void List(int depth, bool Recursive);  // Print the names of all the files
                  //  in the directory
    // void Print(); // Verbose print of the contents
                  //  of the directory -- all the file
                  //  names and their contents.
...
private:
    /*
        ...
        Need to fetch the file from disk using FecthFrom(OpenFile* fileptr!)
        remember to file create an OpenFile* file = new OpenFile(filename)
        filename is the table[i].name.
        Note that entry.name can also be sub-'directory', so 
        its directory structure(information) is also stored on disk "table[i].sectorNumber"
	*/ 

    int tableSize;         // Number of directory entries // number of files created in this FS
    DirectoryEntry *table; // Table of pairs:
                           // <file name, file header location, inUse> 
