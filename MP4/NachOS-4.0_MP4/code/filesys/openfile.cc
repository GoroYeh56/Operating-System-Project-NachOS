...
{
    hdr = new FileHeader;
    hdr->FetchFrom(sector);// For FCB of freeMap: @ sector0.
    seekPosition = 0;
}
...
int OpenFile::ReadAt(char *into, int numBytes, int position)
{
    ...
    // Note: if firstAlgned==TRUE(i.e. position == firstSector*SectorSize):
    //      Simply copy from &buf[0] to into, copy numBytes!
...
int OpenFile::WriteAt(char *from, int numBytes, int position)
{
    ...
    // std::cout<<"From buffer write "<<numBytes<<" bytes to this openfile.\n";
    int i, firstSector, lastSector, numSectors;
    bool firstAligned, lastAligned;
    char *buf;

    // 1. 不得要求在 offset < 0 或 offset > fileLength處寫data !
    if ((numBytes <= 0) || (position >= fileLength)){
        std::cout<<"numBytes: "<<numBytes<<" , position: "<<position<<", fileLength: "<<fileLength<<"\n";
        return 0; // check request
    }
    // 2. 若 起始位置 +  offset  > fileLength處寫data !
    if ((position + numBytes) > fileLength){
        std::cout<<"Too many bytes! Max bytes for this file: "<<fileLength-position;
        numBytes = fileLength - position;
    }
    DEBUG(dbgFile, "Writing " << numBytes << " bytes at " << position << " from file of length " << fileLength);

    // 3. 第一個要寫的 sector  和 最後一個 sector
    // Notes: Logical Block (Sector in nachOS) here.
    firstSector = divRoundDown(position, SectorSize);
    lastSector = divRoundDown(position + numBytes - 1, SectorSize);
    numSectors = 1 + lastSector - firstSector;


    // create a buffer to receive contents you want to write to this openfile
    // First copy from "from"(your data buffer)  to buffer "buf".
    buf = new char[numSectors * SectorSize];
    ...
    // Two flags indicating whether offset alligns with sector boundaries
    firstAligned = (position == (firstSector * SectorSize));   
    lastAligned = ((position + numBytes) == ((lastSector + 1) * SectorSize));

    // read in first and last sector, if they are to be partially modified

    // Read at (firstSector*SectorSize) offset from this file, read SectorSize of bytes into buf.
    if (!firstAligned)
    ...
    // copy in the bytes we want to change  Copy in between!! 
    // Note: if firstAlgned==TRUE(i.e. position == firstSector*SectorSize):
    //       Just simply write from the first bytes of buffer!
    bcopy(from, &buf[position - (firstSector * SectorSize)], numBytes);

    // write modified sectors back to disk.
    // write from buf[(i - firstSector) * SectorSize] , write SectorSize bytes into sector 'hdr->ByteToSector(i * SectorSize)'
     // Notes: Map from Logical Block (Sector in nachOS) => Physical Sectors on-disk.
    for (i = firstSector; i <= lastSector; i++)
