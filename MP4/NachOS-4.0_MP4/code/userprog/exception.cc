...
void ExceptionHandler(ExceptionType which)
{
	...
	case SyscallException:
		...
		// MP4 mod tag
	/* --------------- MP4 --------------*/
		case SC_Create:	// filename, initialSize
			DEBUG(dbgSys, "System call: SysCreate()\n");
			...
			cout << "Create file result is " << status << "\n";
			return;
		...
		case SC_Open: // filename
			...
			cout << "Open file result is " << status << "\n";
			return;
		...
		case SC_Write: // char *buffer, int size, OpenFileId id
			DEBUG(dbgSys, "System call: SysWrite()\n");
			val = kernel->machine->ReadRegister(4); // address of "buffer_name"
			buffer_name = &(kernel->machine->mainMemory[val]);
			size = kernel->machine->ReadRegister(5);
			id = kernel->machine->ReadRegister(6);
			status = SysWrite(buffer_name, size, id);
			kernel->machine->WriteRegister(2, (int)status);
			/* Modify return point */
			{
				/* set previous programm counter (debugging only)*/
